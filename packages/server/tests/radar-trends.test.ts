...
describe('getRadarTrends', () => {
  ...
  it('uses monitored-card samples instead of stale broad radar rows when cards exist', () => {
    ...
    expect(radar.terms.map((term) => term.word)).toContain('交車')
    expect(radar.terms.map((term) => term.word)).not.toContain('法拉利')
    expect(radar.terms.map((term) => term.word)).not.toContain('法拉')
