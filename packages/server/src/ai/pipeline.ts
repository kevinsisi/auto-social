...
import { DEFAULT_VOICE_PROFILE, type PipelineResult, type SourceCandidateInput, type TextGenerator, type VoiceProfile } from './types.js'
...
export class SocialPipeline {
  ...
  async run(candidate: SourceCandidateInput): Promise<PipelineResult> {
    ...
    try {
      ...
      const meme = parseMeme(await this.generate(runner, 'meme', systemInstruction, buildMemePrompt(candidate, safeDraft), assignments))
