...
- [x] 17.E.1 Multi-provider fallback in `sources/threads-fallback-search.ts`: Bing → Google. Bing is tried first because production Google commonly returns `httpservice/retry/enablejs`; Google remains the second provider when Bing is blocked or empty. `scanKeywordCard` also gained the missing fallback wiring that was promised in 17.D.1 but not actually implemented (previously the keyword-scan test was failing).
...
- [x] 17.E.3 `tests/threads-fallback-search.test.ts` (20 cases) covers Google `/sorry/` / `httpservice/retry/enablejs` / unusual-traffic detection, Bing CAPTCHA + Cloudflare Turnstile + ck/captcha URL detection, Bing `/ck/a?...&u=a1...` redirect decoding, provider ordering (Bing preferred, Google fallback), 429/5xx + fetch-throw handling, limit honouring, dedup. `tests/keyword-scan.test.ts` extended (5 cases) for outcome-kind branching including kill-switch still blocking fallback.
- [x] 17.E.4 Add `BRAVE_SEARCH_API_KEY` support as the first non-cache discovery provider. Missing key skips silently; API failure marks `brave` blocked for cooldown; successful API results still pass the same Threads post URL + keyword relevance filters before entering the pipeline. (1.2.65)

### 17.F Quota-aware keyword scheduler

- [x] 17.F.1 Keyword auto scheduler no longer scans every keyword card on every 15-minute tick. It selects only eligible cards: never-scanned cards first, then oldest `lastScanAt`, excluding cards scanned within the minimum interval. Defaults: max 2 cards per tick, min interval 120 minutes. Runtime env overrides: `AUTO_SOCIAL_KEYWORD_SCAN_MAX_PER_TICK`, `AUTO_SOCIAL_KEYWORD_SCAN_MIN_INTERVAL_MINUTES`. (1.2.36)
- [x] 17.F.2 Scheduler selection is quota-aware: each tick caps selected cards by remaining Threads search quota (`dailyLimits.search - todayCounts.search`) and selects zero cards when quota is exhausted. (1.2.36)
- [x] 17.F.3 Scheduler status now reports `lastEligibleCount`, `lastQuotaRemaining`, `lastScannedKeywords`, `maxCardsPerTick`, and `minIntervalMinutes`; Dashboard Workstation shows these fields so operators can see why only a subset was scanned. (1.2.36)
