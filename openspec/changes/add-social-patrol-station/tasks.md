...
- [ ] 11.1 Rework Dashboard to show two tabs: `全網熱門` (candidates with `is_trending = true`) and `我的關鍵字` (candidates with `card_id` set); both sorted by score-by-engagement; both lead into the same per-draft view.
